...
**「瓶頸是磁碟頻寬」是什麼意思？** 傳統佇列對每一則訊息都要個別記帳
（投遞給誰、ack 了沒、何時可刪），成本隨「筆數」線性增長。Kafka 把自己
簡化成「往檔案尾端循序寫、循序讀出去」：不逐筆追蹤、不逐筆刪除
（過期是整個 segment 檔直接砍），讀取大量走 OS page cache 與 zero-copy。
每則訊息的額外成本趨近於零之後，剩下的天花板就是物理極限——
**磁碟每秒能循序寫幾 MB、網卡能送幾 MB**。所以 1 萬筆小訊息和 100 筆大訊息，
只要總 bytes 相同，成本幾乎一樣；也所以第 5 章的容量規劃用 MB/s 而非訊息數來算、
9.3 節堅持資料碟獨立、D9 拒絕有寫放大的 RAID 5/6——全都是在保護這條循序頻寬。
...
本手冊後面會給出很多「好答案」——RF=3、min.insync.replicas=2、acks=all、
三個 AZ⋯⋯但**每個答案都是在回答某個特定的需求問題**。跳過問題直接抄答案，
就是 cargo cult：設定看起來很專業，卻可能為你不需要的保證付出延遲與成本，
或在你真正在乎的地方毫無防護。所以設計的第一步不是打開設定檔，
而是拿下面七個問題去找業務方談。

### 七個必問的需求問題

問的時候用業務語言，不要用 Kafka 術語——「你能接受掉幾筆訂單」
比「你要 acks=all 嗎」能得到誠實得多的答案。

| # | 問業務方的話 | 你實際在決定的事 |
|---|---|---|
| 1 | 「這些資料掉一筆會怎樣？掉一分鐘的量會怎樣？」 | **可靠性等級**：acks / RF / min.insync.replicas |
| 2 | 「系統停多久你會被叫進會議室？」 | **可用性等級**：單叢集 HA 就夠，還是要 DR 叢集；切換自動化到什麼程度 |
| 3 | 「事件發生到下游看到，最慢能接受多久？」 | **延遲目標**：batch/linger/壓縮的取捨；要不要為 p99 犧牲吞吐 |
| 4 | 「同一個客戶的事件，順序錯了會怎樣？」 | **順序性**：key 的選擇、partition 數（順序保證只在單一 partition 內） |
| 5 | 「資料要留多久？有法規要求保留或**刪除**嗎？」 | **保留與合規**：retention/compaction、靜態加密、GDPR 抹除（28.5） |
| 6 | 「哪些團隊會共用？出事時可以互相影響嗎？」 | **隔離**：一個叢集加 quota+ACL，還是拆多叢集（6.4） |
| 7 | 「每秒多少筆、多大、成長多快、尖峰是平常的幾倍？」 | **容量**：磁碟/partition/broker 數——5.1 的五個數字 |
...
| 需求答案 | 設計決策 | 章節 |
|---|---|---|
| 「一筆都不能掉」 | acks=all + RF=3 + min.isr=2 + 冪等，DR 至少 MM2 | 8、22 |
| 「掉幾筆沒關係，但要快」 | acks=1 甚至 0，換延遲與吞吐——**明知故犯才允許** | 8.1 |
| 「停機不能超過 X 分鐘」 | X < 手動切換時間 → 需要 DR 演練排程與 client 切換自動化 | 23.4、24.5 |
| 「順序對單一客戶必須正確」 | key=客戶 ID；partition 數一開始就抓夠（改了順序會亂） | 7.3、5.3 |
| 「法規要求可刪除個資」 | compacted topic + tombstone，並實測抹除延遲 | 28.5 |
| 「多團隊共用」 | 命名規範 + 前綴式 ACL + quota；等級差異大就拆叢集 | 7.1、14、6.4 |
| 「每天 N GB、年成長 M%」 | 5.2 的磁碟公式、5.3 的 partition 決策 | 5 |

### 關鍵架構決策：條件與考量重點

需求給了目標，但同一組需求常有不只一條路。這一節把設計前每個「岔路口」
整理成**可檢驗的條件**（何時該走哪條）與**考量重點**（走了要付什麼代價）——
決策不該靠品味，該靠條件；而每個選擇的代價，最好在選之前就知道。

**D1. 要不要用 Kafka？**（詳見第 1 章）
- 選 Kafka 當：需要重播、多消費者共讀、每秒破萬筆、或系統解耦是主要目的
- 不選當：每天幾千筆、需要優先權插隊、需要逐筆查詢、或團隊沒人能值班維運
- 考量重點：Kafka 是「要持續維運的分散式系統」，導入成本大頭在人不在機器

**D2. 自建還是託管服務（MSK / Confluent Cloud / 各雲廠 Kafka）？**
- 自建當：有合規要求資料不出機房、流量大到託管費用超過人力成本、
  或需要本手冊等級的深度控制（自訂調校、特殊拓撲）
- 託管當：團隊 < 2 個能值班的人、流量中小、想把心力放在應用而非平台
- 考量重點：託管省掉第三、四部的多數工作，但 quota/版本/調校自由度受限，
  且遷移成本高——**用第 5 章算出三年總成本再比價**，別只看月費

**D3. 一個叢集還是多個？**（詳見 6.4）
- 拆叢集當：可靠性等級差異大（金流 vs log）、合規邊界不同、跨地域、
  或單叢集逼近 ~15 台
- 不拆當：以上皆否——用 quota（14.4）+ 前綴 ACL（14.3）做軟隔離就夠
- 考量重點：每多一個叢集，監控/備份/升級/DR 的維運工作全部乘二

**D4. combined mode 還是角色分離？**（詳見 6.1）
- combined 當：≤ 10 節點（本手冊所有練習環境皆是）
- 分離當：> 10 節點，或 metadata 操作延遲已受資料流量影響
- 考量重點：分離後 controller 用小機器即可，但多了一組要照顧的節點；
  滾動重啟順序更要小心（quorum 檢查，見 17 章）

**D5. VM、容器，還是 Kubernetes？**（詳見 F.1、29.1）
- VM 當：沒有成熟的 K8s 平台、或團隊沒有有狀態工作負載的 K8s 經驗（預設選項）
- K8s/Strimzi 當：K8s 已是公司標準平台且有 operator 維運經驗
- 容器（compose）：只用於學習與 CI，不上正式環境
- 考量重點：**別為了 Kafka 才引入 K8s**——故障半徑與除錯路徑都會複雜一級；
  volume 掛載錯誤這種事故（我們實測踩過）只發生在容器化路線

**D6. 部署拓撲：單 AZ、三 AZ，還是跨區 DR？**（由需求第 1、2 題決定）
- 單 AZ + DR 叢集當：延遲極敏感（跨 AZ 的 1–3ms 不可接受）
- 三 AZ 當：標準選項——AZ 級故障仍可讀寫（6.3）
- 再加跨區 MM2 當：第 2 題的答案是「整個機房不見也要能活」（22 章）
- 考量重點：**兩個 AZ 是陷阱**（quorum 湊不出多數，6.3 有論證）；
  跨區複寫是非同步的，RPO > 0 要先讓業務方簽字

**D7. RF 與 min.insync.replicas？**（詳見第 8 章）
- RF=3 + min.isr=2 當：預設——所有「掉了會心痛」的資料
- RF=2 或 1 當：可再生的資料（快取預熱、可重跑的中間結果），且說得出再生程序
- 考量重點：min.isr 必須 < RF（8.2 論證過 =RF 是可用性陷阱）；
  RF 每 +1，磁碟與副本流量同步 +1 份

**D8. partition 數？**（詳見 5.3）
- 下限由吞吐決定：目標吞吐 ÷ 單 partition 實測吞吐（用 perf-test 量，別用猜的）
- 同時受順序性約束：需要順序的單位（如單一客戶）必然同 partition
- 考量重點：**只能加不能減**，而且加了會打亂 key 的落點（7.3）——
  寧可一開始多抓 2–3 倍；但也別上千（每 partition 都吃 FD 與 metadata）

**D9. 磁碟與檔案系統？**（詳見 9.3）
- NVMe/SSD 當：延遲敏感或高吞吐；HDD 僅適合純冷資料高保留場景
- RAID 10 或 JBOD 當：二選一；**RAID 5/6 不要**（寫放大吃掉循序寫優勢）
- 考量重點：Kafka 自己有 RF，磁碟層冗餘是第二層保險——預算緊時
  JBOD + RF=3 是合理組合；ext4/XFS 皆可，掛載記得 noatime（13 章）

**D10. 訊息格式與 Schema 治理？**（詳見 7.4、26.3）
- JSON 當：團隊小、topic 少、schema 變動慢——先活下來
- Avro/Protobuf + Registry 當：跨團隊共用 topic、或 schema 開始頻繁演進
- 考量重點：換格式是全 producer/consumer 的協調工程，越晚換越痛；
  但過早引入 Registry 也是維運負擔——條件是「跨團隊」而不是「想要專業」

### 反向檢核：設計評審這樣用

評審時把表反過來問：**「這個設定回應了哪一條需求？」**
答不出來的設定，要嘛是 cargo cult（刪掉或降級），要嘛是沒被寫下來的需求
（補進需求清單）。每季拿同一張表重新對一次——需求會變，設計要跟著動，
而不是反過來。
...
**如果這份手冊你只讀一節，就讀這一節。**
它回答的是需求清單的**第 1 題**（「掉一筆會怎樣？」）——
讀之前請先確定你拿到了業務方的答案，否則下面的黃金組合只是抄來的。
...
這一部回答的是需求清單的**第 1、2 題**（「掉多少會怎樣？停多久會怎樣？」）——
你需要的備援等級由那兩個答案決定，而不是由技術偏好決定。
...
需求清單**第 2 題**的答案若是「停機不能超過（單機房重建時間）」，你就需要這一章：
單叢集的 RF 擋得住機器故障，擋不住整個機房不見。
